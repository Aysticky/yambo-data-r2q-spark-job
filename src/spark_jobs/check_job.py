...
def check_secrets_manager(secret_name: str, logger: logging.Logger) -> bool:
    ...
    try:
        with log_execution_time(logger, "Check Secrets Manager access"):
            ...
            logger.info(
                f"Successfully accessed secret: {secret_name}",
...
def check_api_connectivity(
    api_manager: StripeAPIKeyManager,
    base_url: str,
...
def check_s3_access(bucket: str, prefix: str, logger: logging.Logger) -> bool:
    ...
    try:
        with log_execution_time(logger, "S3 access check"):
            ...
            if result:
                logger.info(
                    f"S3 access check passed",
                    extra={"bucket": bucket, "prefix": prefix}
                )
            else:
                logger.error(
                    f"S3 access check failed",
                    extra={"bucket": bucket, "prefix": prefix}
                )
...
def check_dynamodb_access(table_name: str, logger: logging.Logger) -> bool:
    ...
    try:
        ...
        logger.error(
            f"DynamoDB access check failed: {e}",
    ...
    try:
        ...
        with log_execution_time(logger, "Spark initialization"):
            spark = (
                ...
                .appName(config.spark.app_name + "-check")
        ...
        checks["s3_access"] = check_s3_access(
        ...
        checks["dynamodb_access"] = check_dynamodb_access(
            config.aws.dynamodb_checkpoint_table,
            logger
        )
        ...
        for check_name, passed in checks.items():
            ...
            logger.log(
                log_level,
                f"{check_name}: {status}",
                extra={"check": check_name, "passed": passed}
            )
        ...
        logger.error(
            f"Check job failed with exception: {e}",
            exc_info=True
        )
        sys.exit(2)
