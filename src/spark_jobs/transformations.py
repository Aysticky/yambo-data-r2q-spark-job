...
def calculate_monetary_metrics(
    ...
    df = df.withColumn(
        f"{amount_col}_usd",
        F.when(F.col(currency_col) == "usd", F.col(amount_col) / 100.0)
        .otherwise(F.col(amount_col) / 100.0)  # Simplified: treat all as USD
    )
...
def filter_test_data(df: DataFrame, patterns: List[str] = None) -> DataFrame:
    ...
    if "receipt_email" in df.columns:
        for pattern in patterns:
            df = df.filter(
                ~F.lower(F.col("receipt_email")).contains(pattern)
            )
    
    # Filter out test customer IDs
    if "customer" in df.columns:
        df = df.filter(
            ~F.col("customer").startsWith("test_")
        )
    ...
    if test_records > 0:
        logger.warning(
            ...
            extra={
                ...
                "test_rate": test_records / initial_count
            }
        )
