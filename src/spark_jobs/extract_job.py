...
from pyspark.sql.types import (
    ...
    LongType, BooleanType, DoubleType, TimestampType
)
...
def fetch_page_partition(
    partition_iter: Iterator[Dict[str, Any]],
...
def transform_charges(df: DataFrame, logger: logging.Logger) -> DataFrame:
    ...
    df = df.withColumn(
        "created_timestamp",
        F.from_unixtime(F.col("created")).cast(TimestampType())
    )
    ...
    df = df.withColumn(
        "refund_rate",
        F.when(F.col("amount") > 0, F.col("amount_refunded") / F.col("amount"))
        .otherwise(0.0)
    )
    ...
    df = df.withColumn(
        "has_customer",
        F.col("customer").isNotNull() & (F.col("customer") != "")
    )
    
    df = df.withColumn(
        "is_fully_refunded",
        F.col("amount_refunded") == F.col("amount")
    )
    
    # Calculate record age
    df = df.withColumn(
        "record_age_days",
        F.datediff(F.current_timestamp(), F.col("created_timestamp"))
    )
    ...
    logger.info(
        f"Transformation complete: {final_count} records",
        extra={"record_count": final_count}
    )
...
def main():
    ...
    try:
        ...
        spark = (
            ...
            .appName(f"{config.spark.app_name}-extract-{run_id}")
        ...
        page_tasks = create_page_tasks(
            ...
            chunk_hours=24  # Daily chunks
        )
        
        logger.info(
            f"Created {len(page_tasks)} page tasks",
            extra={"task_count": len(page_tasks)}
        )
        ...
        with log_execution_time(logger, "Data extraction from API"):
            ...
            records_rdd = tasks_rdd.mapPartitions(
                lambda partition: fetch_page_partition(
                    partition,
                    api_config,
                    config.aws.secrets_manager_secret_name
                )
            )
            ...
            logger.info(
                f"Extracted {record_count} records from API",
        ...
        file_count = count_objects(config.aws.s3_bucket, output_path.replace(f"s3://{config.aws.s3_bucket}/", ""))
        logger.info(
            f"Verification: {file_count} files written to S3",
        ...
        logger.error(
            f"Extract job failed with exception: {e}",
            exc_info=True
        )
